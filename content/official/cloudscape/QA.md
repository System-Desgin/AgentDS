...
> Generated by `pnpm pipeline verify cloudscape` on 2026-08-01. Every
...
- Source type: `npm`
- Distinct colors recovered from source: 237
- Colors grounded in source: **11/11**
  (exact 11 · close 0 · drift 0 · unmatched 0)
- Files read:
  - /index-visual-refresh.json

| Token                       | Value in file | Nearest source color | Verdict |
| --------------------------- | ------------- | -------------------- | ------- |
| `colors.primary`            | #006CE0       | #006CE0              | exact   |
| `colors.on-primary`         | #FFFFFF       | #FFFFFF              | exact   |
| `colors.surface`            | #FFFFFF       | #FFFFFF              | exact   |
| `colors.on-surface`         | #0F141A       | #0F141A              | exact   |
| `colors.on-surface-variant` | #424650       | #424650              | exact   |
| `colors.surface-selected`   | #F0FBFF       | #F0FBFF              | exact   |
| `colors.border`             | #C6C6CD       | #C6C6CD              | exact   |
| `colors.border-strong`      | #8C8C94       | #8C8C94              | exact   |
| `colors.success`            | #00802F       | #00802F              | exact   |
| `colors.warning`            | #855900       | #855900              | exact   |
| `colors.error`              | #DB0000       | #DB0000              | exact   |

## Sign-off

- Automated checks (schema, lint, colour spot-check, license, font substitution) completed by the pipeline.
- Source verification: **passed** — 11/11 colors traced to `npm` source on 2026-08-01 (`pnpm pipeline verify`). CI re-runs this on every content change.
